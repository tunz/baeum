...
fn find_number_range(buf: &Vec<u8>, offset: usize) -> (usize, usize) {
    let mut beg = (1..offset + 1)
        .rev()
        .find(|&x| !is_digit(buf[x - 1]))
        .unwrap_or(0);
    if beg > 0 && buf[beg - 1] == '-' as u8 {
        beg -= 1;
    }

    let end = (offset + 1..buf.len())
        .find(|&x| !is_digit(buf[x]))
        .unwrap_or(buf.len());
...
fn remove_block(buf: &Vec<u8>) -> Vec<u8> {
    ...
    let (to_beg, to_end) = select_block(&buf);
    if to_end - to_beg == buf.len() {
        return buf.clone();
    }
    buf.iter()
        .take(to_beg)
        .chain(buf.iter().skip(to_end))
        .map(|&x| x)
        .collect()
}

fn random_split_point(buf1: &Vec<u8>, buf2: &Vec<u8>) -> usize {
    let mut diffs = buf1.iter()
        .zip(buf2)
        .enumerate()
        .filter(|&(_, (x1, x2))| x1 != x2)
        .map(|(idx, _)| idx);
...
fn cross_over(buf: &Vec<u8>, q: &Vec<Seed>) -> Vec<u8> {
    ...
    if buf_front.len() + buf2_back.len() < 1 {
        return buf.clone();
    }
...
fn test_change_ascii_integer() {
    ...
    assert_eq!(new_buf.iter().find(|&&x| !is_digit(x) && x != '-' as u8),
               None);
}
...
fn test_overwrite_copy_block() {
    ...
    assert_eq!(buf.len(), new_buf.len());
    assert_eq!(new_buf.iter().find(|&&x| x != 0 as u8 && x != 1 as u8),
               None);
}
